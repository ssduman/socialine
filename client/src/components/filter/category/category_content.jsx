...
import { getClubsCategory } from '../../api/apiCall';
...
class Content extends Component {
	...
	componentDidMount() {
		getClubsCategory(this.props.match.params.clubID).then((response) => {
			if (response.status === 500);
			else {
				response.sort((a, b) =>
					a.numberOfUsers < b.numberOfUsers
						? 1
						: a.numberOfUsers === b.numberOfUsers
					...
					{currentClubs.map((element) => {
						return (
							...
							>
								...
								/>
								<div className='card-body' key={'div' + element.subClubId}>
									<h5 className='card-title' key={'h5' + element.subClubId}>
										{element.name}
									</h5>
									<p className='card-text' key={'p' + element.subClubId}>
										{element.description}
									</p>
								</div>
							</div>
